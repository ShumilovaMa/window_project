...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefualt();
      var statusMessage = document.createElement('div'); //блок, в который будем помещать сообщение из объекта

      statusMessage.classList.add('status'); //добавляем стиль блоку

      item.appendChild(statusMessage); //помещаем блок на страницу

      var formData = new FormData(item); //собираем все данный, находящиеся в каждой форме
